...
class EKF:
    ...
    def __init__(self, robot,wheel_val):
        # State components
        self.robot = robot
        self.markers = np.zeros((2,0))
        self.taglist = []
        self.covariance_val = wheel_val
    ...
    def predict_covariance(self, raw_drive_meas):
        n = self.number_landmarks()*2 + 3
        Q = np.zeros((n,n))
        Q[0:3,0:3] = self.robot.covariance_drive(raw_drive_meas)+ self.covariance_val*np.eye(3)
        return Q
        ...
        def drawGrid(canvas):
            ...
            for x in range(0, WINDOW_WIDTH, blockSize):
                for y in range(0, WINDOW_HEIGHT, blockSize):
                    rect = pygame.Rect(x, y+20, blockSize, blockSize)
