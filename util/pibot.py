...
class Alphabot:
    def __init__(self, ip, port, ir=1):
        self.ip = ip
        self.port = port
        self.wheel_vel = [0, 0]
        self.ir = ir
    ...
    ########################################## 
    def set_velocity(self, command, tick=25, turning_tick=15, time=0): 
        ...
        if time == 0:
            pass
            requests.get(
                f"http://{self.ip}:{self.port}/robot/set/velocity?value="+str(l_vel)+","+str(r_vel)+","+str(self.ir))
        else:
            assert (time > 0), "Time must be positive."
            assert (time < 30), "Time must be less than network timeout (20s)."
            requests.get(
                "http://"+self.ip+":"+str(self.port)+"/robot/set/velocity?value="+str(l_vel)+","+str(r_vel)+","+str(self.ir)
                            +"&time="+str(time))
        return l_vel, r_vel
    ...
    def get_ir(self):
        try:
            r = requests.get(f"http://{self.ip}:{self.port}/ir_sensor")
