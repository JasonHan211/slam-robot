...
from slam.ekf_3 import EKF
from slam.robot_3 import Robot
import slam.aruco_detector_3 as aruco
...
class Operate:
    def __init__(self, args):
        ...
        if args.play_data:
            ...
            self.pibot = Alphabot(args.ip, args.port, ir=0)
    ...
    def control(self):       
        if args.play_data:
            ...
            lv, rv = self.pibot.set_velocity(
                self.command['motion'],tick=25,turning_tick=15)
    ...
    def init_ekf(self, datadir, ip):
        ...
        if ip == 'localhost':
            scale /= 2
        fileB = "{}baseline_ccw.txt".format(datadir)  
    ...
    def draw(self, canvas):
        ...
        # paint SLAM outputs
        ekf_view = self.ekf.draw_slam_state(res=(740,620+v_pad),
            not_pause = self.ekf_on)
        canvas.blit(ekf_view, (2*h_pad+480, v_pad))


        robot_view = cv2.resize(self.aruco_img, (480, 360))
        self.draw_pygame_window(canvas, robot_view, 
                                position=(h_pad, v_pad)
                                )

        # canvas.blit(self.gui_mask, (0, 0))
        self.put_caption(canvas, caption='SLAM', position=(2*h_pad+480, v_pad)) # M2
        #self.put_caption(canvas, caption='Detector (M3)',position=(h_pad, 240+2*v_pad)) # M3
        ...
        canvas.blit(count_down_surface, (2*h_pad + 170, 530))
        return canvas
